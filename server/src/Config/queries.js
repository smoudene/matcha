const queries = {
  SELECT: {
    GetUsers: "SELECT * FROM users WHERE id != ? AND verif = 1 AND step = 3",
    GetAllUsers:
      "SELECT  DATE_FORMAT(users.lastSignIn, ' %b %d %Y at %T') as lastSignIn, \
    ...
    GetUserById:
      "SELECT users.*,DATE_FORMAT(users.date_birthday,'%Y-%m-%d') as transDate FROM users \
                    WHERE users.id = ?",
    GetUserByUsername:
      "SELECT users.*,DATE_FORMAT(users.date_birthday,'%Y-%m-%d') as transDate FROM users \
    ...
    getUserLikes:
      "SELECT liker_id,liked_id FROM likesList WHERE liker_id=? OR liked_id=?",
    GetProfilePic:
      "SELECT path FROM pics WHERE user_id = ? AND isProfilePic = 1",
    GetUserTag:
      "SELECT tag FROM tags INNER JOIN useTags ON tags.tag_id = useTags.id_tag \
    ...
    getMessages:
      "SELECT messages.sender,pics.path, messages.message FROM pics,messages \
    ...
    getMatchs:
      "SELECT users.id,users.firstname,users.lastname,pics.path,users.online FROM users,pics \
        WHERE users.id = pics.user_id  AND pics.isProfilePic = 1 AND pics.user_id IN (?)",
    checkBlock:
      "SELECT * FROM blockList WHERE (blocker_id = ? OR blocked_id = ?) AND (blocker_id = ? OR blocked_id = ?)",
    getBlockUser:
      "SELECT id,firstname,lastname FROM users WHERE  id  IN (SELECT blocked_id FROM blockList WHERE blocker_id = ?)",
    getLikedBy:
      "SELECT id,firstname,lastname FROM users WHERE  id  IN (SELECT liker_id FROM likesList WHERE liked_id = ?)",
    getLikeUser:
      "SELECT id,firstname,lastname FROM users WHERE  id  IN (SELECT liked_id FROM likesList WHERE liker_id = ?)",
    getViewProfileList:
      "SELECT id,firstname,lastname FROM users WHERE  id  IN (SELECT viewer FROM viewProfileList WHERE viewed = ?) \
    ...
    checkBlock:
      "SELECT * FROM blockList WHERE (blocker_id = ? OR blocked_id = ?) AND (blocker_id = ? OR blocked_id = ?)",
    getNotif:
      "SELECT users.id, users.username, content, seen FROM notifications,users \
  ...
  INSERT: {
    ...
    insertNotif:
      "INSERT INTO notifications (`by`, receiver, content, seen) VALUES (?, ?, ?, ?)",
    insertMessage:
      "INSERT INTO messages (sender, receiver, message) VALUES (?, ?, ?)",
    reportUser:
      "INSERT INTO reportList (reporter_id, reported_id,date) VALUES (?, ?, NOW())",
    blockUser:
      "INSERT INTO blockList (blocker_id, blocked_id,date) VALUES (?, ?, NOW())",
    likeUser:
      "INSERT INTO likesList (liker_id, liked_id, date) VALUES (?, ?, NOW())",
    reportUser:
      "INSERT INTO reportList (reporter_id, reported_id,date) VALUES (?, ?, NOW())",
    viewProfileUser:
      "INSERT INTO viewProfileList (viewer, viewed, date) VALUES (?,?,NOW())",
  },
  UPDATE: {
    UpToken: "UPDATE users SET vfToken = ? WHERE email = ?",
    ResetPassword: "UPDATE users SET password = ? WHERE vfToken = ?",
    ResetStep: "UPDATE users SET step = 0 WHERE id = ?",
    ...
    UpdateInfo:
      "UPDATE users SET lastname = ?, firstname = ?, gender = ?, Sexual_orientation = ?, date_birthday = ?, age = ?, biography = ? WHERE id = ?",
    UpdateOnline: "UPDATE users SET Online = 1 ,lastSignIn = null WHERE id = ?",
    UpdateOffline:
      "UPDATE users SET Online = 0 ,lastSignIn = NOW() WHERE id = ?",
    UpdateStep: "UPDATE users SET step = ? WHERE id = ?",
    setProfilePic:
      "UPDATE pics SET isProfilePic = 1 WHERE id = ? && user_id = ?",
    resetProfilePic: "UPDATE pics SET isProfilePic = 0 WHERE user_id = ?",
    setFirstProPic:
      "UPDATE  pics SET isProfilePic = 1 WHERE user_id = ? ORDER BY id ASC LIMIT 1",
    UpdateLocation:
      "UPDATE users SET latitude = ? , longitude = ? WHERE id = ?",
    UpdateProfile:
      "UPDATE users SET firstname = ?, lastname = ?, username = ?, email = ?, gender = ?, date_birthday = ?, age = ?, Sexual_orientation = ?, biography = ? \
        WHERE id = ?",
    updateRating:
      "UPDATE users SET rating = rating + ?  WHERE id = ? AND rating <= 5",
